...
#             elif data_type == "C":
#                 options = [f"{k}: {v['en']}" for k, v in value_meaning.items()]
#                 gemini_response = self.gemini_client.generate_response(
#                     evidence_data['question_en'], user_input, options
#                 )
                
#                 if gemini_response == "NO_MATCH":
#                     return {"error": "Unable to determine a valid response using AI."}
                
#                 if gemini_response not in possible_values:
#                     return {"error": "Invalid categorical answer"}
                
#                 answer = gemini_response
#                 self.answers[code] = answer
#                 logger.info(f"Categorical answer recorded for code {code}: {answer}")
#                 return {"message": "Answer recorded successfully"}

#             # Handle Multi-Choice Questions using Gemini
#             elif data_type == "M":
#                 options = [f"{k}: {v['en']}" for k, v in value_meaning.items()]
#                 gemini_response = self.gemini_client.generate_response(
#                     evidence_data['question_en'], user_input, options
...
#                 if gemini_response == "NO_MATCH":
#                     return {"error": "Unable to determine valid responses using AI."}
                
#                 selected_values = gemini_response.split(",")
#                 valid_values = [val.strip() for val in selected_values if val.strip() in possible_values]
                
#                 if not valid_values:
#                     return {"error": "Invalid multi-choice answer"}
                
#                 answer = valid_values
#                 self.answers[code] = answer
#                 logger.info(f"Multi-choice answers recorded for code {code}: {answer}")
#                 return {"message": "Answer recorded successfully"}
    ...
    def submit_answer(self, code, user_input):
        ...
        try:
            ...
            # Handle Categorical Questions using Gemini
                ...
                options = [f"{k}: {v['en']}" for k, v in evidence_data.get("value_meaning", {}).items()]
                gemini_response = self.gemini_client.generate_response(
                    evidence_data['question_en'], user_input, options
                )
            ...
            # Handle Multi-Choice Questions using Gemini
                ...
                possible_values = evidence_data.get("possible-values", [])
                selected_values = [val.strip() for val in gemini_response.split(",")]
                valid_values = [val for val in selected_values if val in possible_values]
